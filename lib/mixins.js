  ...
  */
  ...
  addMenuOption: function(text, callback, className) {
    ...
    hover.alpha=0;
  	...
  	Boton.position.y=this.optionCount*130+this.menuConfig.startY+60;
 ...
 notification(title,boolIns, text, listaDeTuplasConNombreDeBotonYCallback){
  ...
  if( text != undefined){
   ...
   board.addChild(m);
      ...
      right=game.add.text(board.width-50, 100, '>', { font:  '45px RobotoBold', fill: "#013884", align: "center"});
      ...
      left=game.add.text(50, 100, '<', { font:  '45px RobotoBold', fill: "#013884", align: "center"});
  ...
  board.addChild(left);
      ...
      var exit=game.add.text(board.width+board.x-20, board.y-40, 'X', { font:  '30px CondensedLight', fill: 'white', align: "center"});
...
screen1: function(){
    ...
    group1=game.add.group();
      group1.add(game.add.text(0, 0, '1', { font:  '45px AlfaSlab', fill: "#013884", align: "center"}));
    ...
    father= this.game.add.sprite(0,100,'img-1');
    father.scale.setTo(.65,.65);
    a=this.game.add.sprite(father.width,110,'img-1');
    a.scale.setTo(.3,.3);
    b=this.game.add.sprite(father.width+father.width/2,110,'img-1');
    b.scale.setTo(.3,.3);
    c=this.game.add.sprite(father.width,father.height-20,'img-1');
    c.scale.setTo(.3,.3);
    d=this.game.add.sprite(father.width+father.width/2,father.height-20,'img-1');
    d.scale.setTo(.3,.3);
...
screen2: function(){
    ...
    group1=game.add.group();
    group1.add(game.add.text(0, 0, '2', { font:  '45px AlfaSlab', fill: "#013884", align: "center"}));
    group1.add(game.add.text(0, 80, 'Mantené pulsado sobre uno de los seis lugares del arco a donde vas a apuntar ', { font:  '25px TheSansRegular', fill: "#013884", align: "center",wordWrap: true, wordWrapWidth:600}));
    group1.add(game.add.text(220, 180, 'Probabilidades', { font:  '25px TheSansRegular', fill: "#013884", align: "center"}));

    father= this.game.add.sprite(120,220,'arco-n');
    father.scale.setTo(.65,.65);
    a=this.game.add.sprite(160,260,'orange-button');
    a.scale.setTo(1.5,1.5);
    group1.add(game.add.text(165, 265, 'A', { font:  '45px AlfaSlab', fill: "#013884", align: "center"}));

    b=this.game.add.sprite(260,260,'yellow-button');
    b.scale.setTo(1.5,1.5);
    group1.add(game.add.text(265, 265, 'B', { font:  '45px AlfaSlab', fill: "#013884", align: "center"}));

    c=this.game.add.sprite(360,260,'button');
    c.scale.setTo(1.5,1.5);
    group1.add(game.add.text(365, 265, 'C', { font:  '45px AlfaSlab', fill: "#013884", align: "center"}));


    // d=this.game.add.sprite(0,0,'arco-0');
    // d.scale.setTo(.3,.3);
...
screen3: function(){
    ...
    group1=game.add.group();
      group1.add(game.add.text(0, 0, '3', { font:  '45px AlfaSlab', fill: "#013884", align: "center"}));
    group1.add(game.add.text(0, 80, 'Soltá cuando el\n indicador esté en la zona verde de la barra', { font:  '25px TheSansRegular', fill: "#013884", align: "center",wordWrap: true, wordWrapWidth:600}));

    father= this.game.add.sprite(20,280,'pelota');
    father.scale.setTo(1.5,1.5);

    a=this.game.add.sprite(100,250,'linea-potencia');
    // a.scale.setTo(.3,.3);


    b=this.game.add.sprite(0,0,'pelota-sombra');
    b.scale.setTo(2,2);
    c=this.game.add.sprite(230,190,'triangle');
    c.scale.setTo(.5,.5);
    ...
    group1.add(this.game.add.sprite(120,230, myBitmap));
    ...
    //  group1.add(e);
    group1.add(game.add.text(352, 257, 'Potencia', { font:  '15px CondensedRegular', fill: "#013884", align: "center"}));
...
slideScreen(listOfScreens){
  // i++;
  listOfScreens.forEach(function(element){ element.visible=false;}, this);
  listOfScreens[++i].visible=true;
...
//type:: listaDeTupla -> [(nombreDeBoton, Callback)]
 ...
 generateButtonsInSplash: function(listaDeTupla){
       ...
       for(var i=0; i< listaDeTupla.length; i++ ){
              ...
              boton.events.onInputUp.add(listaDeTupla[i][1]);
              nombreBoton.inputEnabled = true;
              nombreBoton.input.useHandCursor = true;
              nombreBoton.events.onInputUp.add(listaDeTupla[i][1]);
              hover.inputEnabled = true;
              hover.input.useHandCursor = true;
              hover.events.onInputUp.add(listaDeTupla[i][1]);
              hover.events.onInputOver.add(function(){hover.input.useHandCursor =false;});
              ...
              // nombreBoton.events.onInputOver.add(onOver, this);
              // nombreBoton.events.onInputOut.add(onOut, this);
              hover.events.onInputOver.add(onOver, this);
              hover.events.onInputOut.add(onOut, this);
 ...
 */
  createSoundGraphics: function(){
    ...
    if( fondoMusic.mute == true){
      musica= game.add.sprite(this.game.width-100,40, 'musica-off');
      musica.scale.setTo(1.20,1.2);
      activateSonido=false;
    }else{
      musica= game.add.sprite(this.game.width-100,40, 'musica-on');
      musica.scale.setTo(1.20,1.2);
      activateSonido=true;
    }
    ...
    // if(!activateSonido){
    //   sonido= game.add.sprite(this.game.width-50,40, 'sonido-off');
    //   sonido.scale.setTo(1.20,1.2);
    // }else{
    //   sonido= game.add.sprite(this.game.width-50,40, 'sonido-on');
    //   sonido.scale.setTo(1.20,1.2);
    // }
    // sonido.inputEnabled = true;
    // sonido.events.onInputDown.add(this.Sonido, sonido);
    // sonido.fixedToCamera=true;
...
sortearFrase: function(num){
    ...
    failPateador=["Afuera!", "A la tribuna!", "A cualquier lado", "A las manos \ndel arquero", "Muy mal patedor", "Brillante el arquero!"];
    goodPateador=["Gooool!", "Golazo!", "Adentro!"];

    frases=[failArquero,goodArquero, failPateador, goodPateador];
