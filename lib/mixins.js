...
// FUNCIONES PARA LOS SPLASH DE NOTIFICACION
 ...
 notification(text){
     ...
     var m=game.add.text(0, 0, text, { font:'30px BitterBold', fill: "black", align: "center",wordWrap: true, wordWrapWidth:400});

    var x=game.add.text(0, 0, "UPS !", { font:  '40px BitterBold', fill: "black", align: "center"});


     var splash = this.game.add.bitmapData(500,x.height+m.height +50);
     var  grd2 = gameBack.context.createLinearGradient(0,0,0,x.height+m.height +50);
     grd2.addColorStop(0,"yellow");
     splash.context.fillStyle = grd2;
     splash.context.fillRect(0,0,this.game.width,this.game.height);
     splash2=this.game.add.sprite(0,0,splash);

     splash2.addChild(x);
     splash2.addChild(m);

     var height=self.calculateHeight(text,400, '20px BitterBold');


 },

 calculateHeight: function(texto, ancho, fuente){
    var m=game.add.text(0, 0, texto, { font: fuente, fill: "black", align: "center",wordWrap: true, wordWrapWidth:ancho});
     alto =m.height;
