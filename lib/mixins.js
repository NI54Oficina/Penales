...
notificationDinamic: function( title, text, boolExitButton, listaDeTuplasConNombreDeBotonYCallback){
 ...
 if(listaDeTuplasConNombreDeBotonYCallback !=undefined){
   ...
   for(var i=0; i< listaDeTuplasConNombreDeBotonYCallback.length; i++){
     var boton=self.createButton(listaDeTuplasConNombreDeBotonYCallback[i][0],listaDeTuplasConNombreDeBotonYCallback[i][1])
      botonesListos.push(boton);
   }
...
createButton: function(nombre, callback, alto, ancho, font){

  var Boton =game.add.group();

  var 	myBitmap = this.game.add.bitmapData(ancho, alto);
  var  grd=myBitmap.context.createLinearGradient(0,0,0,alto/2);
  grd.addColorStop(0,"#0068db");
  grd.addColorStop(0.9,"#0259a3");
  grd.addColorStop(0.93,"#0259a3");
  grd.addColorStop(1,"#013173");
  ...
  var  hoverGra=myBitmap.context.createLinearGradient(0,0,0,alto/2);
  hoverGra.addColorStop(1,"white");
  hoverBit.context.fillStyle=hoverGra;
  hoverBit.context.fillRect(0,0,this.game.height,this.game.width);
  hover = this.game.add.sprite(0,0, hoverBit);
