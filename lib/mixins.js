var mixins = {
  ...
  addMenuOption: function(text, callback, className) {
    ...
    var  childTxt= txt.addChild(game.add.text(0,60, text[1],{
      ...
      txt.position.y=(((background.position.y+background.height)/2))+((this.optionCount*txt.height+ this.optionCount*childTxt.height)/2);

    }else{
      txt.position.y= ((background.position.y+background.height)/2)+this.optionCount*txt.height/2;
    };
